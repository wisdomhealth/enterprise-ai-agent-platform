...
from app.modules.identity.models import StaffUser, UserStatus
...
async def _enqueue_gmail_history_jobs(*, db_session: AsyncSession | None = None) -> None:
    ...
    for connector, knowledge_base_id, sync_state in rows:
        ...
        fingerprint = sha256(
            f"{cursor or 'initial'}\0{page_token or 'first'}".encode()
        ).hexdigest()
        ...
        key = (
            base_key
            if existing is None
            else f"{base_key}:after:{existing.id}:{existing.version}"
        )
...
async def _consume_email_job(job_id: UUID) -> None:
    ...
    async with async_sessionmaker() as db_session:
        ...
        job = await lease_service.claim(job_id, EMAIL_WORKER_ID, lease_seconds=EMAIL_LEASE_SECONDS)
        if job is None:
            return
        await db_session.commit()
        try:
            if job.kind == EMAIL_HISTORY_KIND:
                result = await _consume_history(db_session, job, settings)
                if result:
                    ...
                    await lease_service.complete(
                        job.id, EMAIL_WORKER_ID, expected_version=job.version
                    )
            elif job.kind == EMAIL_CLASSIFY_KIND:
                await _consume_classification(db_session, job, settings)
                await lease_service.complete(job.id, EMAIL_WORKER_ID, expected_version=job.version)
...
async def _consume_classification(
    ...
    ).process_classification(UUID(str(job.payload["work_item_id"])))
...
async def _consume_draft(
    ...
    principal = await _drafting_principal(db_session, item)
    if principal is None:
        raise RuntimeError("no active staff principal has knowledge.read for email drafting")
    grounded = GroundedAnswerService.from_settings(settings)
    await EmailDraftingService(db_session, grounded, principal).generate(item.id)


async def _drafting_principal(db_session, item: EmailWorkItem) -> Principal | None:  # type: ignore[no-untyped-def]
    staff_user = await db_session.scalar(
        select(StaffUser)
        .join(
            ResourceGrant,
            and_(
                ResourceGrant.organization_id == StaffUser.organization_id,
                ResourceGrant.subject_id == StaffUser.id,
            ),
        )
        .where(
            StaffUser.organization_id == item.organization_id,
